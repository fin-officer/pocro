...
class InvoiceValidator:
    ...
    def __init__(self):
        ...
        self.country_currencies = {
            'DE': 'EUR', 'EE': 'EUR', 'FR': 'EUR', 'IT': 'EUR',
            'ES': 'EUR', 'NL': 'EUR', 'AT': 'EUR', 'BE': 'EUR',
            'FI': 'EUR', 'GB': 'GBP', 'PL': 'PLN', 'SE': 'SEK', 'DK': 'DKK'
        }
    ...
    def _validate_business_rules(self, invoice_data: Dict[str, Any]) -> List[ValidationError]:
        ...
        if supplier.get('vat_id'):
            if not self._validate_vat_id(supplier['vat_id'], supplier.get('country_code')):
                errors.append(ValidationError(
                    field='supplier.vat_id',
                    message='Supplier VAT ID format is invalid',
        ...
        if customer.get('vat_id'):
            if not self._validate_vat_id(customer['vat_id'], customer.get('country_code')):
                errors.append(ValidationError(
                    ...
                    message='Customer VAT ID format is invalid',
    ...
    def _validate_business_warnings(self, invoice_data: Dict[str, Any]) -> List[str]:
        ...
        if supplier_country and currency:
            ...
            if expected_currency and expected_currency != currency:
                warnings.append(
                    f"Currency {currency} is unusual for country {supplier_country}, "
    ...
    def _validate_financial_consistency(self, invoice_data: Dict[str, Any]) -> List[ValidationError]:
        ...
        try:
            ...
            if invoice_lines:
                line_total_sum = sum(
                    self._safe_decimal_conversion(line.get('line_total', 0))
            ...
            errors.append(ValidationError(
                ...
                message=f'Error validating financial consistency: {str(e)}',
                value=None
            ))
    ...
    def _calculate_completeness_score(self, invoice_data: Dict[str, Any]) -> float:
        ...
        required_fields = [
            'invoice_id', 'issue_date', 'supplier.name', 'customer.name',
            'total_incl_vat', 'currency_code'
        ]
        
        optional_important_fields = [
            'supplier.vat_id', 'customer.vat_id', 'supplier.country_code',
            'customer.country_code', 'invoice_lines', 'total_excl_vat', 'total_vat'
        ]
