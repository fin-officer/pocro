...
from utils.monitoring import (
    system_monitor, performance_tracker, health_check, 
    track_processing_time, alert_manager
)
...
async def process_batch_invoices(
    ...
    if len(files) > settings.max_batch_size:
        raise HTTPException(
            status_code=400,
            detail=f"Too many files. Maximum {settings.max_batch_size} files per batch"
        )
