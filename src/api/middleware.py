...
from utils.monitoring import system_monitor, performance_tracker
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        try:
            ...
            logger.error(
                f"[{request_id}] Error: {str(e)} ({processing_time:.3f}s)"
            )
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        try:
            ...
            if processing_time > 5.0:
                logger.warning(
                    f"Slow request: {endpoint} took {processing_time:.3f}s "
            ...
            processing_time = time.time() - start_time
            performance_tracker.record_processing_time(
                endpoint, processing_time, False
            )
...
def create_middleware_stack(app, config: Optional[Dict[str, Any]] = None):
    ...
    if config.get("enable_performance_monitoring", True):
        app.add_middleware(
            ...
            enable_detailed_metrics=config.get("detailed_metrics", True)
        )
    
    # Cache control
    app.add_middleware(
        CacheControlMiddleware,
        default_max_age=config.get("default_cache_max_age", 0)
    )
