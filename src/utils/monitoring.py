    ...
    def get_performance_stats(self, operation: Optional[str] = None, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            with self.lock:
                ...
                if operation:
                    recent_data = [
                        entry for entry in self.processing_times
...
def log_system_stats():
    ...
    try:
        ...
        logger.info(f"System Stats - CPU: {stats['cpu']['usage_percent']:.1f}%, "
    ...
    try:
        ...
        with performance_tracker.lock:
            performance_tracker.processing_times = deque([
                entry for entry in performance_tracker.processing_times
                if entry['timestamp'] > cutoff_time
            ], maxlen=1000)
        ...
        for metric_name in list(system_monitor.metrics_history.keys()):
            system_monitor.metrics_history[metric_name] = deque([
                entry for entry in system_monitor.metrics_history[metric_name]
                if entry['timestamp'] > cutoff_time
            ], maxlen=100)
