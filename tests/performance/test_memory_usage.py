...
class TestMemoryUsage:
    ...
    def test_ocr_engine_memory_usage(self, mock_reader, memory_monitor):
        ...
        mock_reader_instance.readtext.return_value = [([[50, 50], [300, 50], [300, 80], [50, 80]], "TEST TEXT", 0.95)]
    ...
    async def test_processor_memory_usage_single_file(self, memory_monitor, test_settings, sample_invoice_image):
        ...
        processor.llm_processor.extract_structured_data = AsyncMock(
            return_value={"invoice_id": "TEST-001", "total_incl_vat": 100.0}
        )
        ...
        for batch_size in batch_sizes:
            ...
            for i in range(batch_size):
                upload_file = UploadFile(
                    filename=f"test_{i}.png", file=BytesIO(sample_image_bytes), content_type="image/png"
                )
    ...
    def test_large_image_memory_usage(self, memory_monitor):
        ...
        sizes = [(1920, 1080), (3840, 2160), (7680, 4320)]  # Full HD  # 4K  # 8K
    ...
    def test_memory_leak_detection(self, memory_monitor):
        ...
        if len(baseline_measurements) >= 3:
            ...
            increases = [
                baseline_measurements[i + 1] - baseline_measurements[i] for i in range(len(baseline_measurements) - 1)
            ]
