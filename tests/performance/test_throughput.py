...
class TestThroughputPerformance:
    ...
    def performance_monitor(self):
        ...
        class PerformanceMonitor:
            ...
            def measure_operation(self, operation_name: str, operation_func, *args, **kwargs):
                ...
                duration = end - start
                self.measurements.append({"operation": operation_name, "duration": duration, "timestamp": start})
        ...
        for width, height in image_sizes:
            ...
            for i, image in enumerate(test_images):
                _, duration = performance_monitor.measure_operation(
                    f"preprocess_{width}x{height}_{i}", preprocessor.preprocess_invoice_image, image
                )
    ...
    def test_ocr_engine_throughput(self, performance_monitor):
        ...
        with patch("src.core.ocr_engine.EASYOCR_AVAILABLE", True):
            with patch("src.core.ocr_engine.easyocr.Reader") as mock_reader:
                ...
                for i in range(20):
                    ...
                    _, duration = performance_monitor.measure_operation(
                        f"ocr_extract_{i}", ocr_engine.extract_invoice_text, test_image
                    )
        ...
        assert (
            stats["throughput_per_second"] > 5.0
        ), f"OCR throughput too low: {stats['throughput_per_second']:.2f} ops/sec"
    ...
    async def test_pipeline_throughput_single_files(self, performance_monitor, test_settings, sample_invoice_image):
        ...
        processor.llm_processor.extract_structured_data = AsyncMock(
            return_value={"invoice_id": "INV-001", "total_incl_vat": 100.0}
        )
        ...
        for i in range(15):
            _, duration = await performance_monitor.measure_async_operation(
                f"pipeline_process_{i}", processor._process_single_image, sample_invoice_image, f"test_{i}"
            )
        ...
        assert (
            stats["throughput_per_second"] > 2.0
        ), f"Pipeline throughput too low: {stats['throughput_per_second']:.2f} ops/sec"
        ...
        for batch_size in batch_sizes:
            ...
            for i in range(batch_size):
                upload_file = UploadFile(
                    filename=f"batch_test_{i}.png", file=BytesIO(sample_image_bytes), content_type="image/png"
                )
            ...
            _, duration = await performance_monitor.measure_async_operation(
                f"batch_process_{batch_size}", processor.process_batch_upload, files
            )
    ...
    def test_sustained_throughput(self, performance_monitor):
        ...
        for batch in range(total_images // batch_size):
            ...
            for i in range(batch_size):
                ...
                _, duration = performance_monitor.measure_operation(
                    f"sustained_{batch}_{i}", preprocessor.preprocess_invoice_image, test_image
                )
    ...
    def test_throughput_with_different_image_sizes(self, performance_monitor):
        ...
        for size_name, width, height in image_configs:
            ...
            for i in range(10):
                ...
                _, duration = performance_monitor.measure_operation(
                    f"{size_name}_{i}", preprocessor.preprocess_invoice_image, test_image
                )
    ...
    def test_memory_vs_throughput_tradeoff(self, performance_monitor):
        ...
        for strategy_name, config in strategies.items():
            ...
            while total_processed < 50:  # Process 50 images total
                ...
                print(f"  Batch {batch_count}: {batch_throughput:.2f} images/sec, " f"Memory: {current_memory:.1f}MB")
        ...
        for name, metrics in results.items():
            print(
                f"{name:<15} {metrics['throughput']:>8.2f}/sec {metrics['peak_memory']:>7.1f}MB "
...
class TestScalabilityLimits:
    ...
    def test_maximum_concurrent_operations(self, performance_monitor):
        ...
        for max_workers in concurrency_levels:
            ...
            results[max_workers] = {"throughput": throughput, "avg_duration": avg_duration, "total_time": total_time}

            print(f"Concurrency {max_workers:2d}: {throughput:6.2f} ops/sec, " f"avg {avg_duration:.3f}s")
        ...
        assert results[4]["throughput"] > results[1]["throughput"] * 0.8, "Concurrency should provide some benefit"
    ...
    def test_file_size_limits(self, performance_monitor):
        ...
        for width, height in sizes:
            ...
            try:
                ...
                print(
                    f"Size {width}x{height} ({pixels/1e6:.1f}MP): "
    ...
    def test_long_running_stability(self, performance_monitor):
        ...
        while time.time() - start_time < target_duration:
            ...
            if operation_count % 50 == 0:
                print(f"Time {elapsed:5.1f}s: {operation_count:4d} ops, " f"current {batch_throughput:.2f} ops/sec")
