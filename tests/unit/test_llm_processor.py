...
class TestLLMProcessor:
    ...
    def test_llm_processor_initialization_vllm(self):
        ...
        with patch("src.core.llm_processor.VLLM_AVAILABLE", True):
            processor = LLMProcessor(model_name="mistral-7b-instruct", quantization="awq", use_vllm=True)
    ...
    def test_llm_processor_initialization_transformers(self):
        ...
        with patch("src.core.llm_processor.TRANSFORMERS_AVAILABLE", True):
            processor = LLMProcessor(model_name="mistral-7b-instruct", quantization="nf4", use_vllm=False)
            assert processor.model_name == "mistral-7b-instruct"
    ...
    async def test_extract_structured_data_different_languages(
        self, sample_invoice_text, sample_llm_response, language
    ):
