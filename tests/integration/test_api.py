...
from ..conftest import assert_valid_invoice_data, create_test_upload_file
...
class TestAPIIntegration:
    ...
    def test_process_invoice_invalid_format(self, client, sample_files):
        ...
        response = client.post(
            ...
            files={
                "file": (
                    sample_files["invalid"].filename,
                    sample_files["invalid"].file,
                    sample_files["invalid"].content_type,
                )
            },
        )
    ...
    def test_process_invoice_image_success(self, mock_processor_class, client, sample_files):
        ...
        response = client.post(
            "/api/v1/process-invoice",
            files={
                "file": (sample_files["image"].filename, sample_files["image"].file, sample_files["image"].content_type)
            },
        )
    ...
    def test_validate_endpoint_valid_data(self, client, sample_extracted_data):
        ...
        response = client.post("/api/v1/validate", json=sample_extracted_data)
    ...
    def test_validate_endpoint_invalid_data(self, client, invalid_invoice_data):
        ...
        response = client.post("/api/v1/validate", json=invalid_invoice_data)
    ...
    def test_large_file_upload(self, client):
        ...
        response = client.post(
            "/api/v1/process-invoice", files={"file": ("large_file.jpg", io.BytesIO(large_content), "image/jpeg")}
        )
    ...
    def test_malformed_json_request(self, client):
        ...
        response = client.post("/api/v1/validate", data="malformed json", headers={"content-type": "application/json"})

        assert response.status_code == 422  # Unprocessable entity

    def test_unsupported_content_type(self, client):
        """Test request with unsupported content type"""
        response = client.post("/api/v1/test", data="some data", headers={"content-type": "application/xml"})
    ...
    def test_error_response_format(self, client):
        ...
        response = client.post(
            "/api/v1/process-invoice", files={"file": ("test.txt", io.BytesIO(b"not an image"), "text/plain")}
        )
    ...
    def test_api_version_consistency(self, client):
        ...
        endpoints = ["/api/v1/config", "/api/v1/languages", "/api/v1/formats", "/api/v1/test"]
