...
class TestPipelineIntegration:
    ...
    async def _setup_mock_behaviors(self, processor):
        ...
        processor.llm_processor.extract_structured_data = AsyncMock(
            return_value={
                ...
                "currency_code": "EUR",
                "supplier": {"name": "Test Company GmbH", "vat_id": "DE123456789", "country_code": "DE"},
    ...
    async def test_process_pdf_file(self, processor_with_mocks, sample_pdf_file):
        ...
        processor_with_mocks.preprocessor.pdf_to_images.return_value = [Mock(spec=["size"], size=(800, 600))]
    ...
    async def test_process_upload_file(self, processor_with_mocks, sample_image_bytes):
        ...
        upload_file = UploadFile(
            filename="test_invoice.png", file=BytesIO(sample_image_bytes), content_type="image/png"
        )
    ...
    async def test_process_batch_upload(self, processor_with_mocks, sample_image_bytes):
        ...
        for i in range(3):
            upload_file = UploadFile(
                filename=f"test_invoice_{i}.png", file=BytesIO(sample_image_bytes), content_type="image/png"
            )
            upload_file.size = len(sample_image_bytes)
    ...
    async def test_merge_extraction_results(self, processor_with_mocks):
        ...
        llm_data = {"invoice_id": "INV-001", "total_incl_vat": 119.0, "invoice_lines": []}

        line_items = [{"description": "Software", "quantity": 1.0, "unit_price": 100.0, "line_total": 100.0}]
    ...
    async def test_validate_financial_totals(self, processor_with_mocks):
        ...
        data = {
            "invoice_lines": [{"line_total": 50.0}, {"line_total": 50.0}],
            "total_excl_vat": 100.0,
            "total_vat": 19.0,
            "total_incl_vat": 119.0,
        }
    ...
    async def test_calculate_quality_metrics(self, processor_with_mocks):
        ...
        ocr_result = {"avg_confidence": 0.9, "total_elements": 10}
    ...
    async def test_combine_page_results_multiple_pages(self, processor_with_mocks):
        ...
        page_results = [
            ...
            {"structured_data": {"invoice_id": "INV-001", "invoice_lines": [{"description": "Item 2"}]}},
        ]
    ...
    async def test_performance_multiple_files(self, processor_with_mocks, sample_image_bytes):
        ...
        for i in range(5):
            upload_file = UploadFile(
                filename=f"perf_test_{i}.png", file=BytesIO(sample_image_bytes), content_type="image/png"
            )
            upload_file.size = len(sample_image_bytes)
            files.append(upload_file)
    ...
    async def test_memory_usage_monitoring(self, processor_with_mocks, sample_invoice_image):
        ...
        for i in range(5):
            result = await processor_with_mocks._process_single_image(sample_invoice_image, f"memory_test_{i}")
            assert "structured_data" in result
    ...
    async def test_concurrent_processing(self, processor_with_mocks, sample_invoice_image):
        ...
        tasks = []
        for i in range(3):
            task = processor_with_mocks._process_single_image(sample_invoice_image, f"concurrent_{i}")
            tasks.append(task)
